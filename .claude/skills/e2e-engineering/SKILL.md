...
- User invoked `/e2e-engineering qa`, OR `queue.json` has any Task `status:pending-qa` → run **QA sign-off session** ([human-qa](../../../skills/e2e-engineering/post-impl/human-qa.md) multi-Task mode): walk every pending-qa Task's [qa-signoff.md](../../../skills/e2e-engineering/schemas/qa-signoff.md), sign off (→ `done`), route findings through [triage](../../../skills/e2e-engineering/impl/triage.md) into new queue Tasks, and forward each Task's [flow-retro](../../../skills/e2e-engineering/schemas/flow-retro.md) §Skill-improvement candidates upstream to the e2e-engineering repo (three distinct lanes — ADR 0027). Offer this first when pending-qa work exists. Stop here when done.

...
Recurring, repo-wide scan that surfaces architecture-deepening opportunities as [refactor candidates](../../../skills/e2e-engineering/schemas/refactor-candidate.json.md) → [triage](../../../skills/e2e-engineering/impl/triage.md) → human-picked refactor Tasks in [queue.json](../../../skills/e2e-engineering/schemas/queue.json.md). **Incremental** via the [scan ledger](../../../skills/e2e-engineering/schemas/scan-ledger.json.md): scan only areas not already covered + unchanged. **Code is NEVER auto-refactored** — surface candidates only (same wall as map-codebase §5). Deeper than map-codebase §5's change-scoped byproduct: a dedicated [architecture-scout](../../agents/architecture-scout.md) per area.
...
Read [scan-ledger.json](../../../skills/e2e-engineering/schemas/scan-ledger.json.md). Per area, hash its file subtree and decide:
...
Dispatch ONE [architecture-scout](../../agents/architecture-scout.md) per eligible area, in parallel (`Agent`, read-only). Inject: [constitution](../../../skills/e2e-engineering/constitution.md) + (when present) ARCHITECTURE.md §1–§5 (§Index for offset/limit) + the area's file list. Each returns a [refactor-candidate manifest](../../../skills/e2e-engineering/schemas/refactor-candidate.json.md): `{ area, verdict, candidates[] }`.
...
Route all surfaced candidates through [triage](../../../skills/e2e-engineering/impl/triage.md) (externally-sourced intake — NOT born ready-for-agent). Present the candidate set (priority + blastRadius + behaviorPreserved) to the human. Human picks which become refactor Tasks:
- Picked → append refactor Task to [queue.json](../../../skills/e2e-engineering/schemas/queue.json.md) (`status:needs-spec`, `selected:false`, refactor-shaped — runs FULL flow incl. its own PRD, ADR 0012/0029). `behaviorPreserved` seeds the safety-net acceptance criterion.

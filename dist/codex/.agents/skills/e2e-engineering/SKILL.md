...
Durable project docs at repo ROOT: `CONTEXT.md` (glossary), [constitution](constitution.md) (generic engineering standards), `ARCHITECTURE.md` (project-specific structure + conventions — schema: [architecture](schemas/architecture.md)), `DESIGN.md` (project design system — register, tokens, components — schema: [design](schemas/design.md)). ARCHITECTURE.md + DESIGN.md written ONLY in human phases (pre-impl seed + post-impl human-QA amend); implementation loop reads them, never writes. See ADR 0013, ADR 0030.
...
- User invoked `/e2e-engineering qa` (explicit arg) → skip the menu; run the **QA sign-off session** ([human-qa](post-impl/human-qa.md) multi-Task mode) over **every** `status:pending-qa` Task (same session as the Pending-QA bucket below, but no selection step): read `queue.json`, each pending Task's [qa-signoff.md](schemas/qa-signoff.md), and `progress.txt`; present full Manual test script (steps + expected, not title summary); sign off (→ `done`) only after human results. Stop here when done.
- **Bare `/e2e-engineering` with a populated queue** — `queue.json` has ≥1 Task in `todo` OR `pending-qa` (and Step 1 found no single `in-progress` Task to resume) → **Front-door menu (ADR 0028).** Do NOT auto-route and do NOT fall through to a fresh spec. Ask the two-way fork, STOP for answer (end with `WAITING: pick (1) new task or (2) existing queue`):
  ...
  - **(2) Existing queue** → present the queue as up to **three labeled buckets** (show only non-empty ones), each its own selectable checklist with priority + dependsOn; **STOP and wait** for the human's selection (end with `WAITING: select Task(s) to work — <bucket>`). Selection routes by bucket:
    ...
    - **Pending QA sign-off** — `status:pending-qa` Task. Select → **QA sign-off session** ([human-qa](post-impl/human-qa.md) multi-Task mode) over the selected Tasks: present full Manual test script (steps + expected), sign off (→ `done`) only after human results, route findings through [triage](impl/triage.md) into new queue Tasks, and forward each Task's [flow-retro](schemas/flow-retro.md) §Skill-improvement candidates upstream to the e2e-engineering repo (three distinct lanes — ADR 0027).
...
5. [design](pre-impl/design.md) — only if the task bears UI (the `design?` gate). Sets register (Brand vs Product), seeds/records `DESIGN.md` (schema: [design](schemas/design.md)) against the [ui-design standard](standards/ui-design.md). Human-phase-written; flight reads it. Greenfield drafts (`<!-- SEED -->`); brownfield records reality + flags deviations.
6. [to-prd](pre-impl/to-prd.md) — convert grill-with-docs notes into formal PRD → writes `prd.json`. Owns own interview step (no double-interview). Refactor-shaped stories allowed. Captures testing-decisions → test-cases.
   - **Plan with expert agents → architecture-aware PRD.** Before finalizing, consult advisor agents against `ARCHITECTURE.md` + `DESIGN.md` + [constitution](constitution.md). **Inline default** for normal PRDs. **Manifest fan-out for high-risk** (schema-heavy, security, cross-service, complex UX, or explicit user request): write expert-consult manifest → spawn standard Codex `worker` agents in parallel and inject the matching canonical expert specs (`backend-architect`, `dba`, and — UI lens on the spec pre-build — `product-designer`) from `skills/e2e-engineering/agents/` → collect findings → apply to PRD. backend-architect/dba + `frontend-reviewer` review built slices in [/e2e-flight](../e2e-flight/SKILL.md) (product-designer advises here, frontend-reviewer reviews there).
...
2. **Land in [queue.json](schemas/queue.json.md) as `ready-for-flight`** (ADR 0029) — two cases:

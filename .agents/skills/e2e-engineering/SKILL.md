...
3. **Fan-in (orchestrator, serial — sole writer):** per returned summary, run per-slice review's two ordered stages, then merge-readiness check, then merge.
   ...
   - **Bounce ceiling (token-runaway guard).** Stage-1 or stage-2 bounce capped at **3 round-trips**. Still failing after 3 → STOP: mark story `blocked`, append `## Blocked` in progress.txt, keep draining. _Legacy single-Task inline path only; superseded for `/e2e-flight` by ADR 0035's review convergence loop (cap 4, review findings never produce `blocked`, residue rides `followups.json`)._
